...
def print_header(lpath):
    ...
    # changed to max_col_sheet1 in lace 4
    for c in range(1, max_col_sheet1 + 1):
        mastersheet.cell(row=max_row_master, column=c).value = \
            loadingsheet.cell(row=1, column=c).value
        ...
        for rows in range(2, maxrows + 1):
            if select == 1:
                for col in range(1, 3):
                    cellvalue1 = \
                        activesheet.cell(row=rows, column=col)
                    if str(cellvalue1.value) == str(keysearch[0]):
                        if a == 1:  # printing name once
                            mastermaxrow = mastersheet.max_row
                            mastermaxrow = mastermaxrow + 1
                            for r in range(1, 4):
                                mastersheet.cell(row=mastermaxrow, column=r).value = \
                                    activesheet.cell(row=rows, column=r).value
                                r = r + 1
                            a = a + 1
                        mastermaxrow = mastersheet.max_row + 1
                        k = 4
                        for temp in range(4, maxcol + 1):
                            mastersheet.cell(row=mastermaxrow,
                                             column=k).value = \
                                activesheet.cell(row=rows,
                                                 column=temp).value
                            k = k + 1
...
def Inputsearchkey():
    d1a = input(
        " Do you want to: A)\033[1m Search By PS Number "
        ":\033[0m B) \033[1mSearch By User Name :\033[0m [A/B]? :  ")
    if d1a == "A" or d1a == "a":
